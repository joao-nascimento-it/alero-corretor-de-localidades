...
import {
  createQueryAllIncorretaLocalidades,
  createQueryFirstIncorretaLocalidade,
  IncorretaLocalidade,
} from "./createIncorretaLocalidadeRepository.ts";

Deno.test("createQueryAllIncorretaLocalidades", async (t) => {
  await t.step("Should query all", async () => {
    const database = new Fake<IncorretaLocalidade>([{
      municipio: "municipio",
      estado: "estado",
    }]);

    const queryAllItems = createQueryAllIncorretaLocalidades({
      path: "file.json",
  ...
  await t.step("Should query first", async () => {
    ...
    const queryFirstItem = createQueryFirstIncorretaLocalidade({
      path: "file.json",
      safeReadJson: Fake.createFakeSafeReadJson(database),
    });
